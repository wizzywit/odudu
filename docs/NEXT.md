...
**`clients_service_subject_fk` (0005_subjects.sql) is a third instance of
the same trap**: an unrestricted composite `ON DELETE SET NULL` on
`(realm_id, service_subject_id)` would null `realm_id` on `clients`
alongside it, and a service subject's delete would fail `clients`'s own
`NOT NULL` rather than detach it. The fix is the same column-list form:
