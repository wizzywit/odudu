...
| P4a | Token exchange (RFC 8693)                                        | 45–65 h   | token exchange (RFC 8693) as a grant at stage 3 of the token pipeline — `subject_token` and `actor_token`, audience and scope narrowing, `act` and nested `act` carrying the delegation chain, and impersonation distinguished from delegation by a per-client permission that is off by default — with `access_token`, `refresh_token` and `id_token` accepted and issued, the generic `:jwt` type refused on a recorded rationale, `may_act` enforced where present, and the SAML types placed against P8; an exchanged token that dies with the session its subject token belonged to, proven at `/introspect` and `/userinfo` both, since exchange would otherwise launder a session-bound token into one no logout can reach; and, since adding a grant is the next change to grant selection in `issueTokens`, `/token` refusing a grant its client is not registered for, which nothing checks today, on every client-authentication path; a client permitted to exchange creatable without `psql`; a clause table under `docs/protocols/` that `pnpm trace` reads; cross-tenant RLS probes green                                                                                                                                                                                                                                                                                                                                                              |
| P4c | Admin API                                                        | 95–125 h  | a system tenant bootstrapped by `odudu seed admin` with a single-use password and a forced change; an admin API at `/admin` serving both a tenant's own admins and a system admin holding `manage-tenants`, with capability re-resolved per request and proven by a capability matrix over every route; a tenant's settings, SMTP (credential behind the same key-encryption interface as a signing key), authentication flow with `required` and `conditional` made to differ without changing an existing tenant's behaviour, and claim-mapper bindings feeding a per-tenant discovery document, all manageable without `psql`; clients created, amended and disabled, with `client.enabled` read by `/userinfo`, `/introspect` and all three exchange branches through one shared predicate and the affected transcripts re-run; a subject's sessions listed by subject — orphans included — and ended individually; two-phase key rotation with signing selected by algorithm, so an algorithm change strands nobody and retirement is refused while it would; a service account provisioning users under `manage-users` alone; opaque-cursor pagination with `Link` headers and no totals; admin mutations audited in the mutating transaction, with a redacted diff and retention through `reap`; OpenAPI published under an amended ADR 0007, proven to cover every route; `docs/admin-paths.md` with every transcript executed; cross-tenant RLS probes green |
| P4e | Authentication and token audit events                            | 40–60 h   | the `audit_events` table P4c built for it carrying authentication and token events as well as admin mutations — a login attempt and its outcome, a second factor offered and answered, a token minted, refreshed, exchanged and revoked, a session created and ended, and a credential's lifecycle — registration, email verification, password reset and change, TOTP and passkey enrolment, a fresh set of recovery codes — with the nullable actor meaning "the subject this happened to" rather than "the administrator who did it", and `GET /admin/tenants/{tenant}/audit` filtering by `event_type` so an operator can read one kind without the other; retention through the same `audit_retention_days` setting and the same position in `REAP_ORDER`, with no second window to keep in step; ADR 0036's follow-up closed — `requested_userinfo_claims` threaded onto the rotated grant, so a refresh no longer widens what `/userinfo` returns, which is the defect that ADR leaves open and names against this phase; and the cross-tenant refusal `docs/NEXT.md` recorded as deferred given an answer (ADR 0037) — a bearer token naming an unrecognised issuer was refused before its signature could be checked and so wrote no row, and recording it needed a bound that an unauthenticated caller could not use to append one per request; cross-tenant RLS probes green                                                                              |
| P4d | Admin and account consoles                                       | 55–85 h   | an ADR choosing what the consoles are built with, written before the first line of console code; an admin console driving the API P4c publishes — the audit trail P4e fills among it, read by event type — with no capability reachable in the UI that the API does not expose; `POST /admin/tenants/{tenant}/clients` honouring or refusing every field `PATCH` amends, `audiences` among them, which it silently drops today; a constraint on tenant names, which accept `/` and so let one tenant's issuer nest under another's; CI caching by Turborepo, landed before the first Playwright increment, since `verify` already takes about fifteen minutes; an account console for self-service, including a fresh set of recovery codes the subject can ask for before the old set is spent and a warning when fewer than a configured number remain; tenant import and export; the five functional layers carried by packages outside the server for the first time, with the boundary suite's fixtures extended to prove `service-is-a-leaf` is not over-broad; a session shown against the device it belongs to, which needs the stable browser identifier ADR 0033 derives and the spec's decision 1 defers, and which also closes the orphan that decision admits; Playwright green                                                                                                                                                                          |
...
| P9  | Authorization services                                           | 100–150 h | policy evaluation and UMA 2.0; a resource server as a first-class thing — the per-audience scope model and `/introspect`'s "may introspect" entitlement, named together because they are one missing abstraction and building half of it twice is the failure this row exists to prevent: today every scope is a tenant's and any client authenticating at `/introspect` may introspect any token in its tenant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| P10 | Extensibility: third-party providers                             | 40–65 h   | a third-party provider loads without a rebuild; a claim mapper an operator supplies loads the same way, so the registry P4c lets a tenant rebind can also be extended — `standardClaimMappers` and the role and group mappers beside it are the whole vocabulary today, and nothing a tenant writes can add to it; an `EventListener` an operator supplies — the registry §8 names, which nothing implements today — receives a tenant's audit events as they are written, so a SIEM or a webhook sees what `audit_events` records without polling `GET /admin/tenants/{tenant}/audit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| P11 | HA, clustering, performance                                      | 60–100 h  | three replicas behind a load balancer, with migrations taking an advisory lock so that simultaneous boots cannot race, and every bound held in one process's memory — the per-origin throttle, the `client_secret` limiter and `auditRefusalBudget` among them — either shared or documented as multiplied by the replica count; documented p99 for `/token` and `/userinfo`, which owes a deadline to the outbound DNS lookups those two paths make and today do not bound                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| P12 | Operational readiness                                            | 40–70 h   | a versioned image published from a tagged release, with the release process written down; secrets sourced from somewhere other than the process environment, through the interface §5 already puts the key-encryption key behind; backup and restore guidance proven by restoring into an empty database and completing a login against the restore; one configured issuer base for the deployment, so `iss` is minted from configuration rather than derived per request — the absence of which forced the admin API's audience to a fixed URN, and which `ODUDU_PUBLIC_BASE_URL` already half supplies                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| P13 | FAPI 2.0, sender-constrained tokens and the deferred grants      | 85–130 h  | the OIDF FAPI 2.0 Security Profile plan passes, which requires pushed authorization requests (RFC 9126) and DPoP (RFC 9449) or mTLS-bound tokens; a requested `acr_values` the current session cannot satisfy forces reauthentication rather than being ignored; a device-code client (RFC 8628) completes a login on a second device; `private_key_jwt` and `tls_client_auth` accepted at `/introspect` and `/revoke`, which only the two password methods reach today; RFC 7523's clauses tracked in `docs/protocols/`, the one RFC Odudu implements whose clauses the traceability matrix cannot see; RFC 7592's client configuration endpoint, a client reading, amending and deleting its own registration against the `registration_access_token` RFC 7591 already issues — collected here because that token is a client-authentication mechanism and this is the phase that reworks client authentication, not because dynamic registration is unfinished                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |

Total: roughly 1380–2045 hours.

After P4b, at roughly 745–1060 hours, an application can adopt Odudu: every
...
### P4 became four phases, then five

Found on 2026-09-23, brainstorming P4. The row carried fourteen independent
deliverables at 155–230 hours, against P3b's 135–165, which §11 already
treated as the largest it would tolerate whole. P2, P3 and P10 each split
for less.

Brainstorming P4c split it again. Audit events divide cleanly in two: the
admin mutations the API itself writes, which cannot be deferred past the API
that produces them, and the authentication and token events, which share the
table and nothing else. The second became **P4e**, so P4c ships the table
shaped for both and writes only the rows it owns.

The order is **P4a → P4c → P4e → P4d → P4b**, and the letters do not read in
execution order. `P4b` was spent on theming on 2026-09-17 when P10 split,
and it is cited in fifteen passages here and seven in ADR 0030; an accepted
ADR is corrected by appending rather than by editing what it said, so
renaming it would falsify a record that was true when written. The wart is
cheaper than the falsification.

**Why the grant goes first, and why the other protocol-shaped work does
not.** Token exchange depends on nothing the other three build: its
permissions are per-client metadata on `client_oidc_config`, which the
dynamic registration endpoint P3a shipped already writes. Configurable claim
mappers and a configurable authentication flow look like protocol work and
are not — "configurable" is a surface, not a mechanism, and shipping either
before the admin API leaves it settable by `psql` alone, which is the state
P4 exists to end. They are P4c's, delivered through the thing that makes
them configurable.

**The citation rule, and where it stopped working.** Roughly 130 bare `P4`
references exist across these documents, and the P2 split established that
the answer is a written rule rather than 130 edits — a row renumbered
wrongly is invisible for good, since `pnpm trace` skips `deferred:` rows
entirely.

That precedent is now partly obsolete, which this split found by running the
build rather than by reasoning about it.
`tests/docs/phase-references.test.ts` — added after P2 split — resolves every
phase `README.md` and `docs/request-paths.md` cite against the rows of this
table, so deleting the `P4` row made twelve bare citations fail. No written
rule can satisfy that check; they were resolved one at a time.

For the prose the check does not read — `docs/NEXT.md` and the archived
phase specs — a bare `P4` named the next phase still open, which the rule
tracks as each split phase closes rather than fixing once: it meant P4c
until P4c shipped, P4e until P4e did, and **means P4d** now, unless it
concerns token exchange, the grant allowlist, the admin API, audit events,
the grant's UserInfo claims, theming or client branding.

Two things the split does not cost, both checked rather than assumed:
`tests/docs/not-implemented-placement.test.ts` accepts any `P<digits><letter>`,
so the build check needs no edit, and nothing under `docs/protocols/` defers
a clause past P3, so no traceability row moves.

The total rose by 40–45 hours. That is not an estimating correction applied
to the same work: splitting forced the token-exchange profile to be decided
rather than left to the phase, and a full RFC 8693 profile is larger than
the 20–30 hours the grant was carried at when it was one line inside a
console phase.
...
That cost was paid on 2026-09-23 rather than guarded against: P4 became four
phases and the grant became the first of them, P4a, where nothing can skip
it because nothing else is in it. The subsection below has the split.
...
amended row, and what establishes it.

**The events.** Every action in `AUDIT_ACTIONS` has a production writer,
which `tests/lint/audit-vocabulary-coverage.test.ts` checks by literal and
shows able to fail. Each writer is proven by driving the door a user
reaches, in `packages/protocol-oidc/tests/audit-*.int.test.ts`,
`packages/protocol-admin/tests/audit-access.int.test.ts` and the
`account`/`authn-flows` suites, and `docs/request-paths.md` shows the rows
each door writes against a running stack: a refused login, a session's two
rows, a refused token request, registration, a reset, and one sign-in's
trail read back through `GET /admin/tenants/{tenant}/audit`.

**The actor.** `actor_subject_id` is the subject the event happened to and
`actor_tenant_id` the tenant it belongs to, which is the row's own except
for `token.foreign_issuer`. The second half was found unimplemented while
the end-to-end transcript was captured — every non-admin row carried a
null — and closed in `auditRepository` before the phase did.

**`event_type` filtering.** Validated as an enum of the six types, shown in
`docs/admin-paths.md` and in the trail above.

**Retention.** Unchanged, and so in step by construction: `reap`'s
`audit_events` statement keys on the tenant's `audit_retention_days` and
never on `event_type`.

**ADR 0036's follow-up.** `requested_userinfo_claims` is on the grant row
(migration `0070`), and a refresh answers `/userinfo` with the claims the
code was issued for; ADR 0036's amendment records it closed.

**The cross-tenant refusal.** Answered by ADR 0037: a genuine token from
another tenant served here is verified against that tenant's keys and then
recorded; a forged one, or one naming an issuer not served here, is a
`warn` line. Every refusal row at `/token`, `/revoke` and `/introspect`
spends a per-client `auditRefusalBudget`.

**Cross-tenant RLS probes.** `auditRepository` is probed with a foreign
`tenant_id`, and every writer's integration file carries a probe.

**Every clause the row names was delivered.** The estimate rose from 35–55
...
**What the whole-branch review closed.** The session cookie carried its
session's id, which is also every token's `sid`; it now carries a secret as
well (the P4e spec's §16, migration `0071`), and the logout confirmation a
token keyed by that secret. Login-step rows name a digest of the
`auth_session_id`; a replayed code writes one revocation row however often
it is replayed; refused token exchanges are rows; and `request_id` is
recorded as a correlation, not evidence (ADR 0037's second and third
amendments).
`docs/phases/p4e.md` has each.
