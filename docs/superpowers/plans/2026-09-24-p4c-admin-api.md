...
- **The eight-line cap is a ceiling, not a target.** Comments carry only what the code cannot express: delete one that restates the code or the name, and delete one that argues a design decision already recorded in the spec or an ADR — a one-line pointer naming that document beats three paraphrasing it. Keep what a reader cannot recover from the code, at the shortest length that still carries it. **The comments embedded in this plan's code blocks are indicative, not verbatim**: they were written to explain the design to you, and several proved too wordy for the code they sit above. Cut them as you transcribe.
...
**Spike A answered (2026-09-24): the approach holds, no fallback needed.** `Ajv2020` from `ajv/dist/2020` wires into a Fastify plugin scope via `setValidatorCompiler` and genuinely enforces 2020-12 semantics — a `prefixItems` tuple accepts `['a', 1]` and rejects `[1, 'a']`, so it is not draft-07 silently ignoring the keyword. **Two things Task 3.1 must do:** add `ajv` and `ajv-formats` as **direct** dependencies of `protocol-admin` (they are currently only transitive, via `@fastify/ajv-compiler`), and scope the compiler to the admin plugin rather than the root instance.

**Spike B answered (2026-09-24): the approach holds, with one addition.** A `setErrorHandler` registered inside the admin plugin is fully encapsulated — the OIDC routes' error bodies and content types are provably untouched. **Task 3.2 must also register `setNotFoundHandler` inside the admin plugin**: without it a 404 under `/admin` falls through to the root handler and answers `application/json` rather than problem details, which is the same defect one door along.
