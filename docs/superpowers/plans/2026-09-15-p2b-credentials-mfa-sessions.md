...
- **A task that adds persisted state to a session must enumerate every existing outcome that leaves that session alive.** Not "add a test" — list them, then decide for each. This plan introduced a persisted `satisfied` set without asking what it does to the two outcomes that deliberately leave an authentication session unconsumed for a retry: the `id_token_hint` mismatch, where the right End-User signs in against the same parked request, and the `verify_email` refusal, where the same session retries once the address is verified. Both are prose contracts, one with a live transcript in `docs/request-paths.md`, and neither had a test — so nothing in the increment's own exit criteria could have caught the regression.
...
Count with `Array.from(candidate).length` — the spread form `[...candidate]` trips this repository's `@typescript-eslint/no-misused-spread`, so the two are semantically identical and only one of them lints. Return every violation. The service takes no `tx` and no clock — it is a leaf.
...
**This task must pin the write side of the flow engine's resumption condition.** The engine persists a satisfied factor only when a further step remains, and that branch is currently untested and untestable: with `password` the only applicable authenticator, every flow completes in one step, so deleting the `recordSatisfied` call leaves the whole suite green. `advance` takes no injectable registry, so it is not unit-testable either. A real OTP authenticator makes it reachable — assert that a password success followed by an applicable OTP step writes `password` into `satisfied`, and that deleting the write breaks it.

**This task must bind `satisfied` to a subject.** The flow engine records satisfied factors by authenticator name, and the authentication session records no subject, so with two real factors nothing checks that step two's `subjectId` is step one's — the journey's answer becomes whoever completed the last step. Unreachable while only `password` runs; reachable the moment an OTP authenticator exists, which is this task. Bind the subject across steps and test that a second factor completed by a different subject does not complete the first subject's login.

Also: a factor satisfied on an attempt whose login was then **refused downstream** must not let a retry skip it. The single-step case is handled by not persisting when the flow completes in one step; with two factors that no longer covers it, so decide explicitly whether a refused outcome clears `satisfied` or whether the subject binding above is sufficient, and say which in the report.
...
- Create: `packages/db/drizzle/0039_authentication_sessions_webauthn_challenge.sql`
- Modify: `packages/db/drizzle/meta/_journal.json`
- Modify: `packages/authn-flows/src/schema/authentication-sessions.ts`
...
- [ ] **Step 0: Write the migration that gives the challenge somewhere to live**

The challenge is stored against the authentication session, not the browser,
and `authentication_sessions` has no column for it.

```sql
-- packages/db/drizzle/0039_authentication_sessions_webauthn_challenge.sql
-- The challenge issued for a WebAuthn ceremony, held server-side so the
-- browser cannot choose what it will later be asked to have signed. Null
-- whenever no ceremony is in flight; cleared when one completes, so a
-- replayed response finds nothing to match against.
ALTER TABLE authentication_sessions ADD COLUMN webauthn_challenge text;
```

Journal entry as usual. `authentication_sessions` already has RLS and a policy
from migration 0032 — adding a column restates neither.
...
`packages/db/drizzle/`, not this table.
