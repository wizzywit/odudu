...
discovery advertisement (§2.1). What logout revokes, and why access tokens
...
| ------ | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2      | MUST   | when both `client_id` and `id_token_hint` are present, the OP verifies that the Client Identifier matches the one used when issuing the ID Token         | —                         | deferred: P3 — `subjectOfIdTokenHint` verifies only that this realm issued the hint (`AUDIENCE_UNCHECKED`), and `client_id` is used solely to resolve the client's registered `post_logout_redirect_uris`; comparing it against the hint's `aud` belongs with the per-client logout registration P3 introduces |
| 2      | MUST   | the value of `post_logout_redirect_uri` has been previously registered with the OP                                                                       | `OIDC-RPINITIATED-3-01`   | covered                                                                                                                                                                                                                                                                                                        |
| 2      | SHOULD | `post_logout_redirect_uri` uses the `https` scheme                                                                                                       | —                         | deferred: P3 — nothing constrains the scheme of a value a client registers, and the registration path that would apply such a policy arrives with client management                                                                                                                                            |
...
| 2      | MUST   | the OP supports the HTTP `GET` and `POST` methods at the Logout Endpoint                                                                                 | —                         | deferred: P3 — `GET` carries the logout request; the `POST` route (`view/routes/logout.ts`) exists for the confirmation form and refuses a body with no `session_id`, so a Form-Serialized logout request is not yet accepted                                                                                  |
...
| 3.1    | SHOULD | registered `post_logout_redirect_uris` use the `https` scheme                                                                                            | —                         | deferred: P3 — the same missing registration-time scheme policy as §2's own row on the request parameter                                                                                                                                                                                                       |
...
| 4      | MUST   | information that failed to validate is not used                                                                                                          | `OIDC-RPINITIATED-3-01`   | covered                                                                                                                                                                                                                                                                                                        |
...
### `POST` at the Logout Endpoint answers the form, not an RP

§2 requires both HTTP methods: "OpenID Providers MUST support the use of
the HTTP `GET` and `POST` methods defined in RFC 7231 at the Logout
Endpoint", with an RP free to Form-Serialize the request parameters into a
`POST` body. Odudu's `POST` handler is the confirmation form's, and a body
carrying no `session_id` is answered with the unauthenticated page rather
than read as a logout request — so an RP that posts `id_token_hint` and
`post_logout_redirect_uri` gets a 400 where the specification requires the
same treatment `GET` gets. The row is deferred rather than accepted
because nothing about the design prevents it: the handler has to try the
confirmation path only when the form field is present, and fall through to
`handleLogoutRequest` with the body's parameters otherwise.
