...
have something to attach to when P2a, P2b and P3a land.
...
stays `deferred: P3a`, since the `claims` parameter itself is still P3a's.
...
| 2       | MUST   | `auth_time` is present when requested as an Essential Claim via the `claims` parameter                                                                                                                                                                                                         | —                      | deferred: P3a — the `claims` request parameter is deferred to P3a                                                                                                                                                                                                                                                                     |
...
| 3.1.2.1 | MUST   | with `prompt=none`, an error is returned if the client lacks pre-configured consent for the requested claims                                                                                                                                                                                   | —                      | deferred: P3a — no consent screen exists yet                                                                                                                                                                                                                                                                                          |
...
| 3.1.2.1 | SHOULD | with `prompt=consent`, the end-user is prompted for consent before information is released                                                                                                                                                                                                     | —                      | deferred: P3a — no consent screen exists yet                                                                                                                                                                                                                                                                                          |
| 3.1.2.1 | MUST   | with `prompt=consent`, an error (typically `consent_required`) is returned if consent cannot be obtained                                                                                                                                                                                       | —                      | deferred: P3a — no consent screen exists yet                                                                                                                                                                                                                                                                                          |
...
| 3.1.2.2 | MUST   | when `sub` is requested with a specific value, a positive response is sent only if the identified end-user has an active session or was authenticated as a result of this request                                                                                                              | —                      | deferred: P3a — `sub` can only be requested with a specific value through the `claims` request parameter, which arrives with the per-client claim machinery and consent                                                                                                                                                               |
...
| 3.1.2.4 | MUST   | once authenticated, the authorization server obtains an authorization decision before releasing information to the client                                                                                                                                                                      | —                      | deferred: P3a — no consent screen exists yet                                                                                                                                                                                                                                                                                          |
...
| 3.1.2.6 | MAY    | `consent_required` is returned under `prompt=none` when consent is required                                                                                                                                                                                                                    | —                      | deferred: P3a — no consent screen exists yet                                                                                                                                                                                                                                                                                          |
...
| 16.7    | MAY    | the server requires the client's request to be digitally signed for non-repudiation                                                                                                                                                                                                            | —                      | deferred: P13 — signed requests rely on request objects (§6), which are deferred to P13                                                                                                                                                                                                                                               |
| 16.7    | SHOULD | the server validates the digital signature of a signed request                                                                                                                                                                                                                                 | —                      | deferred: P13 — signed requests rely on request objects (§6), which are deferred to P13                                                                                                                                                                                                                                               |
...
| 16.18   | SHOULD | the authorization server clearly identifies long-term grants to the user during authorization                                                                                                                                                                                                  | —                      | deferred: P3a — no consent screen exists yet                                                                                                                                                                                                                                                                                          |
