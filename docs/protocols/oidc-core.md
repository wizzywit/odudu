...
builds, and the `claims` parameter itself is still unbuilt once P3a's
per-client machinery and consent screen land — but the claim itself is
cheap for P1 to support unconditionally: Odudu's session/authn-flow layer
...
satisfied. `prompt=consent` needed a consent screen and per-client
registration data that did not exist until dynamic client registration
landed; both shipped in **P3a**, and those rows are `covered`. **P3b** for
what P3a's registration data then feeds — signed and encrypted UserInfo
