...
| 2.3    | MAY    | logout requests are sent to the logged-in RPs in parallel                                                                                             | —                         | deferred: P3b — `sendLogouts` (`packages/protocol-oidc/src/usecase/send-logouts.ts`) drains one claimed row at a time; each delivery is already independently bounded by `responseTimeoutMs`, but nothing sends two at once                                                                                                                                                    |
