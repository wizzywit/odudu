...
its delivery require is `deferred: P3`, which §11 of the design spec names
...
| ------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1    | MAY    | the OP advertises `backchannel_logout_supported` as `true`                                                                                            | —                         | deferred: P3 — no back-channel logout metadata is published yet; the value would claim a capability the OP does not have                                                          |
| 2.1    | SHOULD | the OP also registers `backchannel_logout_session_supported`                                                                                          | —                         | deferred: P3 — it arrives with the Logout Token that the `sid` it advertises would travel in                                                                                      |
...
| 2.2    | MUST   | a registered back-channel logout URI is an absolute URI                                                                                               | —                         | deferred: P3 — no client metadata holds a back-channel logout URI yet, so there is none to validate                                                                               |
| 2.2    | MUST   | a query component in a registered back-channel logout URI is retained when further query parameters are added                                         | —                         | deferred: P3 — nothing composes a back-channel logout request yet                                                                                                                 |
| 2.2    | MUST   | a registered back-channel logout URI includes no fragment component                                                                                   | —                         | deferred: P3 — no client metadata holds a back-channel logout URI yet, so there is none to validate                                                                               |
| 2.2    | SHOULD | `backchannel_logout_uri` uses the `https` scheme                                                                                                      | —                         | deferred: P3 — the registration-time scheme policy arrives with the registration                                                                                                  |
| 2.2    | MAY    | `backchannel_logout_uri` uses the `http` scheme where the Client Type is confidential and the OP allows it                                            | —                         | deferred: P3 — the registration-time scheme policy arrives with the registration                                                                                                  |
| 2.2    | SHOULD | `backchannel_logout_session_required` is registered alongside it, and honoured when the logout URI is used                                            | —                         | deferred: P3 — the OP's half, including a `sid` in the Logout Token when a client demands one, needs the Logout Token first                                                       |
| 2.3    | SHOULD | the OP keeps track of the set of logged-in RPs, so that it knows which to contact at their back-channel logout URIs                                   | —                         | deferred: P3 — a session records the grants issued from it but not the clients to notify; that index is part of delivering a Logout Token                                         |
| 2.3    | MAY    | logout requests are sent to the logged-in RPs in parallel                                                                                             | —                         | deferred: P3 — nothing sends them yet                                                                                                                                             |
| 2.4    | MUST   | a Logout Token carries `iss`, the Issuer Identifier                                                                                                   | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MUST   | a Logout Token carries `aud`                                                                                                                          | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MUST   | a Logout Token carries `iat`                                                                                                                          | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MUST   | a Logout Token carries `exp`                                                                                                                          | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MUST   | a Logout Token carries `jti`, a unique identifier for the token                                                                                       | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MAY    | a Logout Token carries `sub`                                                                                                                          | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MAY    | a Logout Token carries `sid`                                                                                                                          | —                         | deferred: P3 — the `sid` value exists and is already in every ID Token (§2.1); the token that would carry it does not                                                             |
| 2.4    | MUST   | a Logout Token carries an `events` Claim whose value is a JSON object containing the member name `http://schemas.openid.net/event/backchannel-logout` | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MUST   | that member's value is a JSON object                                                                                                                  | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | SHOULD | that member's value is the empty JSON object                                                                                                          | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MUST   | a Logout Token contains either a `sub` or a `sid` Claim, and may contain both                                                                         | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.4    | MUST   | a `nonce` Claim is not present in a Logout Token                                                                                                      | —                         | deferred: P3 — no Logout Token is minted yet; the claim exists on Odudu's ID Tokens, which is exactly why this prohibition has to be written into the minting rather than assumed |
| 2.4    | MAY    | a Logout Token contains other Claims                                                                                                                  | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
...
| 2.4    | MUST   | a Logout Token is signed, and may also be encrypted                                                                                                   | —                         | deferred: P3 — no Logout Token is minted yet; the realm's own ID Token keys are what would sign it                                                                                |
| 2.4    | SHOULD | an encrypted Logout Token replicates the `iss` claim in its JWT header parameters                                                                     | —                         | deferred: P3 — Odudu encrypts no tokens at all; encrypted responses arrive with the per-client registration that selects them                                                     |
| 2.4    | SHOULD | Logout Tokens are explicitly typed, with a `typ` header parameter of `logout+jwt`                                                                     | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
| 2.5    | MUST   | the back-channel logout request's `POST` body includes a `logout_token` parameter containing a Logout Token for the RP                                | —                         | deferred: P3 — nothing sends a back-channel logout request yet                                                                                                                    |
| 2.5    | MAY    | the `POST` body contains other values alongside `logout_token`                                                                                        | —                         | deferred: P3 — nothing sends a back-channel logout request yet                                                                                                                    |
...
| 2.5    | SHOULD | retransmission is delayed an appropriate amount of time when a previous transmission may have failed recoverably                                      | —                         | deferred: P3 — nothing sends a back-channel logout request yet                                                                                                                    |
| 2.5    | SHOULD | in every other case, a back-channel logout request is not retransmitted                                                                               | —                         | deferred: P3 — nothing sends a back-channel logout request yet                                                                                                                    |
...
| 4      | SHOULD | the implementation makes clear which kinds of Relying Party it can log out                                                                            | —                         | deferred: P3 — there is nothing to describe until a Logout Token is delivered                                                                                                     |
| 4      | SHOULD | Logout Tokens are given short expiration times, preferably at most two minutes                                                                        | —                         | deferred: P3 — no Logout Token is minted yet                                                                                                                                      |
