...
| 2.2    | MAY    | `backchannel_logout_uri` uses the `http` scheme where the Client Type is confidential and the OP allows it                                            | —                         | n/a: `parseClientMetadata` (packages/protocol-oidc/src/service/client-metadata.ts) refuses `http` unconditionally — no Client Type exception is implemented, so this MAY is never exercised |
