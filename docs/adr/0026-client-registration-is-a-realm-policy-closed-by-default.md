# 0026 — Client registration is a realm policy, closed by default
...
Every realm toggle P2a and P2b added — `registration_allowed`,
...
`disabled`, `open`, `token` — on `realms.client_registration_policy`, which
...
`disabled` and an unknown realm are two different facts a client-supplied
`{realm}` cannot be permitted to distinguish. Discovery and JWKS already
...
- `disabled` (default): the endpoint answers 404, and `registration_endpoint`
  ...
  something that refuses everything. The same treatment a disabled realm
...
realm open. A token presented and rejected is refused outright rather than
...
- **Open by default.** Rejected for the same reason every other realm
...
`RealmLookup` row it already fetches to answer 404/200 — no second query.
The registration endpoint's own 404 for `disabled` and for an unknown realm
