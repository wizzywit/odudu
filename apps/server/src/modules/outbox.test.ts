...
import { OUTBOX_JITTER_FRACTION, outboxModule, outboxSchedule } from '#/modules/outbox';
import { type OutboxPass } from '#/modules/outbox';
...
const ONE_TICK_MS = INTERVAL_MS * (1 + OUTBOX_JITTER_FRACTION);
...
interface Call {
  readonly deps: SendPendingDeps;
  readonly now: Date;
  readonly options: SendPendingOptions;
}

function recordingPass(outcome: SendPendingOutcome): {
  pass: OutboxPass;
  calls: readonly Call[];
} {
  const calls: Call[] = [];
...
const DELIVERED: SendPendingOutcome = { ran: true, sent: 2, failed: 1 };

describe('whether the server sends queued mail on its own schedule', () => {
  it('schedules on the configured interval with a jittered tail', () => {
    const decision = outboxSchedule(loadConfig({ ...MINIMAL, ...APP_URL }));

    expect(decision).toEqual({
      scheduled: true,
      intervalMs: INTERVAL_MS,
      jitterMs: INTERVAL_MS * OUTBOX_JITTER_FRACTION,
    });
  });

  it('declines when the schedule is switched off', () => {
    const decision = outboxSchedule(
      loadConfig({ ...MINIMAL, ...APP_URL, ODUDU_OUTBOX_ENABLED: 'false' }),
    );

    expect(decision).toEqual({ scheduled: false, why: 'switched-off' });
  });

  // `sendPending` refuses without ODUDU_APP_DATABASE_URL in every
  // environment, while the boot guard demands it only in production.
  // Decided here, once, rather than rediscovered by a pass that throws on
  // every tick forever.
  it('declines rather than scheduling a pass that can only refuse', () => {
    const decision = outboxSchedule(loadConfig(MINIMAL));
...
describe('outboxModule', () => {
  ...
  it('waits for migrations, so a pass cannot run against a half-built schema', () => {
    expect(outboxModule(DEPS).dependsOn).toEqual(['database']);
  });

  it('gives the pass the clock and the options the context carries', async () => {
    const log = recorder();
    const { pass, calls } = recordingPass(DELIVERED);
    const ctx = context({ ...APP_URL, ODUDU_OUTBOX_BATCH_SIZE: '7' }, log.logger);
    const module = outboxModule(DEPS, pass);
  ...
  it('reports what a pass sent and what it could not', async () => {
    const log = recorder();
    const module = outboxModule(DEPS, recordingPass(DELIVERED).pass);
    ...
    expect(log.lines).toContainEqual({
      level: 'info',
      payload: { sent: 2, failed: 1 },
      message: 'outbox pass complete',
    });
  });

  // Every few seconds, forever: a line per tick would bury everything else
  // in the log. A pass that found nothing says nothing.
  it('says nothing about a pass with an empty queue', async () => {
    const log = recorder();
    const module = outboxModule(DEPS, recordingPass({ ran: true, sent: 0, failed: 0 }).pass);

    await module.start?.(context(APP_URL, log.logger));
    await vi.advanceTimersByTimeAsync(ONE_TICK_MS * 3);
    await module.stop?.();

    expect(log.lines.map((line) => line.message)).not.toContain('outbox pass complete');
  });
  ...
  // itself after the throw.
  it('keeps sending after a pass throws', async () => {
    const log = recorder();
    let passes = 0;
    const module = outboxModule(DEPS, () => {
      passes += 1;
      return passes === 1
        ? Promise.reject(new Error('the transport is unreachable'))
  ...
  it('starts no timer and runs nothing when the schedule is switched off', async () => {
    ...
    await module.start?.(context({ ...APP_URL, ODUDU_OUTBOX_ENABLED: 'false' }, log.logger));

    expect(vi.getTimerCount()).toBe(0);
    expect(log.lines).toHaveLength(1);
    expect(log.lines[0]?.level).toBe('info');
    expect(log.lines[0]?.message).toContain('ODUDU_OUTBOX_ENABLED=false');
  ...
  // One line, at boot, naming both halves of the way out: the variable that
