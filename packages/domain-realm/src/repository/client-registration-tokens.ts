...
export function clientRegistrationTokenRepository(tx: RealmScopedDatabase) {
  return {
    ...
    // first's decrement as soon as it commits. `realmId` is carried in the
    ...
    // by Postgres to the UPDATE itself — a caller-supplied realmId is not
    // what stops a token minted in another realm from being spent here.
    async spend(realmId: string, token: string): Promise<boolean> {
      ...
      const rows = await tx
        ...
        .where(
          and(
            eq(clientRegistrationTokens.realmId, realmId),
