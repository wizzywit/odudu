...
async function seedUser(tx: TenantScopedDatabase, tenantId: string): Promise<string> {
...
describe('the database enforces what the claim promises', () => {
  ...
  it('updates no profile in another tenant', async () => {
    const tenantA = newId();
    const tenantB = newId();
    const subjectInA = await withTenant(app.db, tenantA, (tx) => seedUser(tx, tenantA));
    ...
    const stillA = await withTenant(app.db, tenantA, (tx) =>
...
describe('markEmailVerified', () => {
  it('flips emailVerified for the named user', async () => {
    ...
    const reread = await withTenant(app.db, tenantId, (tx) =>
  ...
  it('cannot verify a user in another tenant', async () => {
    ...
    await expect(
      withTenant(app.db, tenantB, (tx) => userRepository(tx).markEmailVerified(subjectInA)),
    ).rejects.toThrow(/not found/);
...
describe('SQL/TypeScript parity for the shape-constrained columns', () => {
  it('users_birthdate_shape accepts exactly what isValidBirthdate accepts', async () => {
    ...
    for (const candidate of cases) {
      accepted.push(await acceptedByColumn(tenantId, subjectId, 'birthdate', candidate));
    }
  ...
  it('users_locale_shape accepts exactly what isValidLocale accepts', async () => {
    ...
    const accepted: boolean[] = [];
    for (const candidate of cases) {
      accepted.push(await acceptedByColumn(tenantId, subjectId, 'locale', candidate));
    }
  ...
  it('users_zoneinfo_shape accepts exactly what isValidZoneinfo accepts', async () => {
    ...
    const accepted: boolean[] = [];
    for (const candidate of cases) {
      accepted.push(await acceptedByColumn(tenantId, subjectId, 'zoneinfo', candidate));
    }
