...
// exists in this realm at all, and whether this call was the one that moved
    ...
    async recordFailure(
      ...
      for (let attempt = 0; attempt < CAS_ATTEMPTS; attempt++) {
        ...
        const result = await tx.execute(sql`
          WITH target AS (
            SELECT s.realm_id, s.id FROM subjects AS s WHERE s.id = ${subjectId}
          ), written AS (
            INSERT INTO login_failures
                   (realm_id, subject_id, failure_count, first_failure_at, last_failure_at, locked_until)
            SELECT target.realm_id, target.id, ${next.failureCount},
                   ${first}::timestamptz, ${at}::timestamptz, ${until}::timestamptz
              FROM target
            ON CONFLICT (realm_id, subject_id) DO UPDATE
