...
const PATH = '/tenants/:tenant/clients-registrations/openid-connect';
...
export function registerClientRegistrationRoute(
  ...
  app.post<{ Params: { tenant: string } }>(PATH, async (request, reply) => {
    ...
    switch (outcome.kind) {
      ...
      // RFC 7591 §3.2.2 has no error code of its own for a tenant at its
      // client cap; invalid_client_metadata is the closest fit — the
      // request is refused for a property of the tenant's state, not a
      // malformed field, but the response shape is the same one a client
      // already has to handle.
      case 'at_capacity':
        return reply.code(403).send({
          error: 'invalid_client_metadata',
          error_description: 'this tenant has reached its client registration limit',
        });
      case 'ok':
